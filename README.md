...
fetch_coop_data.py ── pdfplumberのテーブル抽出 + overrides.json による補完
   │
   ├─ 成功 → data/YYYY_M.json(全期間), data/coop_data.json(掲載中の月のみ)を更新してコミット
...
| `current_pdfs.json` | 生協サイトに現在掲載中のPDF一覧(coop_data.jsonの範囲を決める) |
| `data/YYYY_M.json` | 月ごとの営業時間データ(全期間アーカイブ) |
| `data/coop_data.json` | 公開ページが参照するデータ。読み込み速度のため掲載中PDFの月のみ |
| `errors/` | 未解決の抽出エラーのアーティファクト |
| `pdf/` | 取得したPDFのアーカイブ |
| `docs/` | GitHub Pages(公開ページ) |
| `scripts/stop_hook.sh` | 自動修正時のStopフック(test.py通過まで終了をブロック) |
...
| `claude_auto_fix.yml` | fetchからのdispatch / `extraction-error` ラベル付与 | Claude Code (常に最新のSonnet) がoverrides.jsonを修正してPR作成。修正ブランチ(`claude-fix/issue-N-<run>`)はワークフローが事前作成し、Stopフックがtest.py通過まで終了をブロックする |
